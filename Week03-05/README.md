...
### Motion model (week 3)
Using the drive signals, you can estimate the resulting location of the robot. **Please fill in the code segment in [Robot.py](slam/Robot.py) ([line 27](slam/Robot.py#L27)) to complete this motion model.** As shown in [slam/Measurements.py](Measurements.py), this motion model will be used by the SLAM function.
...
As shown in [aruco_detector.py](slam/aruco_detector.py) and [slam/Measurements.py](Measurements.py), the ARUCO marker information will be used by the SLAM function.
